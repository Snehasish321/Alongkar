...
export interface BlossomSkyProps {
  className?: string;
  speed?: number; // default 22
}

export const BlossomSky: React.FC<BlossomSkyProps> = ({
  ...
  useEffect(() => {
    ...
    // Quad geometry (single full-screen triangle covering [-1,-1] to [3,-1], [-1,3])
    ...
    const uNscale = gl.getUniformLocation(program, 'u_nscale');
    const uDirv = gl.getUniformLocation(program, 'u_dirv');
    const uNoise = gl.getUniformLocation(program, 'u_noise');

    gl.uniform1i(uNoise, 0);

    // Colors from "Blossom sky" preset (ordered from high lightness to dark/deep):
    // 1. ROSE RICE #FFEFF6 -> [1.0, 0.937, 0.965] (high)
    // 2. WISTERIA MIST #F2C4DC -> [0.949, 0.769, 0.863] (main)
    // 3. PLUM SMOKE #C9A6E8 -> [0.788, 0.651, 0.910] (mid)
    // 4. CLEAR HANADA #7E8FD0 -> [0.494, 0.561, 0.816] (low)
    gl.uniform3f(uHigh, 1.0, 0.937, 0.965);
    gl.uniform3f(uMain, 0.949, 0.769, 0.863);
    gl.uniform3f(uMid, 0.788, 0.651, 0.910);
    gl.uniform3f(uLow, 0.494, 0.561, 0.816);

    // Physics parameters for sky motion
    gl.uniform1f(uNscale, 0.868);
    gl.uniform1f(uWarp, 0.235);
    gl.uniform1f(uWind, 0.144);
    gl.uniform2f(uDirv, 1.0, 0.0);
